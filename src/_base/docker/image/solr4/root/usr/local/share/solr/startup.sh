...
if [ "$CORE_EXISTS" -ne 0 ]; then
  curl -IX GET "http://localhost:8983/solr/admin/cores?action=CREATE&name=$SOLR_CORE_NAME&instanceDir=$SOLR_CORE_NAME&config=solrconfig.xml&schema=schema.xml&dataDir=data"
fi
