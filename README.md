...
force pushed to the individual harness repositories' `0.7.x` branch.
...
When ready to tag a release, make a new branch from the `0.7.x` branch for the changelog entries:
...
5. On each downstream repository:
   1. Tag the latest `0.7.x` branch push as the release version.
