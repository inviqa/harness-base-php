...
force pushed to the individual harness repositories' `0.11.x` branch.
...
When ready to tag a release, make a new branch from the `0.11.x` branch for the changelog entries:
...
3. Run the following docker command to generate the changelog, replacing `0.11.0` with the version number as needed:
  ```bash
  docker run -e CHANGELOG_GITHUB_TOKEN="$CHANGELOG_GITHUB_TOKEN" -it --rm -v "$(pwd)":/usr/local/src/your-app -v "$(pwd)/github-changelog-http-cache":/tmp/github-changelog-http-cache ferrarimarco/github-changelog-generator --user inviqa --project harness-base-php --exclude-labels "duplicate,question,invalid,wontfix,skip-changelog" --release-branch 0.11.x --future-release 0.11.0
  ```
...
7. Adjust the version for each framework's README.md: `sed -i'' s/v0\.10\.0/v0.11.0/ src/*/README.md src/*/docs/*.md  src/*/docs/*/*.md`
...
1. Tag the release version with `git tag 0.11.0 -m "v0.11.0"`
2. Push the tag to the repository: `git push origin 0.11.0`
...
6. Create a "Github Release" for this repository and downstream repositories, pasting in the changelog for the release:
   - https://github.com/inviqa/harness-base-php/releases/new?tag=0.11.0&title=0.11.0&target=0.11.x
   - https://github.com/inviqa/harness-php/releases/new?tag=0.11.0&title=0.11.0&target=0.11.x
   - https://github.com/inviqa/harness-akeneo/releases/new?tag=0.11.0&title=0.11.0&target=0.11.x
   - https://github.com/inviqa/harness-drupal8/releases/new?tag=0.11.0&title=0.11.0&target=0.11.x
   - https://github.com/inviqa/harness-magento1/releases/new?tag=0.11.0&title=0.11.0&target=0.11.x
   - https://github.com/inviqa/harness-magento2/releases/new?tag=0.11.0&title=0.11.0&target=0.11.x
   - https://github.com/inviqa/harness-spryker/releases/new?tag=0.11.0&title=0.11.0&target=0.11.x
   - https://github.com/inviqa/harness-symfony/releases/new?tag=0.11.0&title=0.11.0&target=0.11.x
   - https://github.com/inviqa/harness-wordpress/releases/new?tag=0.11.0&title=0.11.0&target=0.11.x

### Post-release actions

If the next release does not make sense to be in the current 0.11.x branch:
...
2. Adjust references from 0.11.x to 0.12.x:
  ...
  grep -FR '0.11.x' . | grep -v dist/
  # Edit resulting files
  ```
3. Adjust references in this file from 0.11.0 to 0.12.0:
  ```bash
