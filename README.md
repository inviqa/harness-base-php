...
force pushed to the individual harness repositories' `0.12.x` branch.
...
If the next release does not make sense to be in the current 0.12.x branch:

1. Create a new branch:
  ```bash
  git checkout -b 0.13.x
  ```
2. Adjust references from 0.12.x to 0.13.x:
  ```bash
  grep -FR '0.13.x' . | grep -v dist/
