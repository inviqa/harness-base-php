...
force pushed to the individual harness repositories' `1.3.x` branch.
...
When ready to tag a release, make a new branch from the `1.3.x` branch for the changelog entries:
...
3. Run the following docker command to generate the changelog, replacing `1.3.0` with the version number as needed:
  ```bash
  docker run -e CHANGELOG_GITHUB_TOKEN="$CHANGELOG_GITHUB_TOKEN" -it --rm -v "$(pwd)":/usr/local/src/your-app -v "$(pwd)/github-changelog-http-cache":/tmp/github-changelog-http-cache githubchangeloggenerator/github-changelog-generator --user inviqa --project harness-base-php --exclude-labels "duplicate,question,invalid,wontfix,skip-changelog" --release-branch 1.3.x --future-release 1.3.0
  ```
...
7. Adjust the version for each framework's README.md: `sed -i '' s/1\.1\.2/1.3.0/ README.md src/*/README.md src/*/docs/*.md  src/*/docs/*/*.md`
...
1. Tag the release version with `git tag 1.3.0 -m "v1.3.0"`
2. Push the tag to the repository: `git push origin 1.3.0`
...
6. Create a "Github Release" for this repository and downstream repositories, pasting in the changelog for the release:
   - https://github.com/inviqa/harness-base-php/releases/new?tag=1.3.0&title=1.3.0&target=1.3.x
   - https://github.com/inviqa/harness-php/releases/new?tag=1.3.0&title=1.3.0&target=1.3.x
   - https://github.com/inviqa/harness-akeneo/releases/new?tag=1.3.0&title=1.3.0&target=1.3.x
   - https://github.com/inviqa/harness-drupal8/releases/new?tag=1.3.0&title=1.3.0&target=1.3.x
   - https://github.com/inviqa/harness-magento1/releases/new?tag=1.3.0&title=1.3.0&target=1.3.x
   - https://github.com/inviqa/harness-magento2/releases/new?tag=1.3.0&title=1.3.0&target=1.3.x
   - https://github.com/inviqa/harness-spryker/releases/new?tag=1.3.0&title=1.3.0&target=1.3.x
   - https://github.com/inviqa/harness-symfony/releases/new?tag=1.3.0&title=1.3.0&target=1.3.x
   - https://github.com/inviqa/harness-wordpress/releases/new?tag=1.3.0&title=1.3.0&target=1.3.x

### Post-release actions

If the next release does not make sense to be in the current 1.3.x branch:

1. Create a new branch:
  ```bash
  git checkout -b 1.4.x
  ```
2. Adjust references from 1.3.x to 1.4.x:
  ```bash
  grep -FR '1.4.x' . | grep -v dist/
  grep -FR '1.3.x' . | grep -v dist/
  # Edit resulting files
  ```
3. Adjust references in this file from 1.3.0 to 1.4.0:
  ```bash
  grep -FR '1.4.0' README.md
  grep -FR '1.3.0' README.md
  # Edit resulting files
  ```
...
6. Run a deployment of the 1.3.x branch so the branches now exist in the downstream repositories:
  ```bash
  ./deploy
  ```
...
[version specific upgrade instructions]: https://github.com/inviqa/harness-base-php/blob/1.3.x/UPGRADE.md
