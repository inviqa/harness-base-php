...
3. Run the following docker command to generate the changelog, replacing `0.10.2` with the version number as needed:
  ```bash
  docker run -e CHANGELOG_GITHUB_TOKEN="$CHANGELOG_GITHUB_TOKEN" -it --rm -v "$(pwd)":/usr/local/src/your-app -v "$(pwd)/github-changelog-http-cache":/tmp/github-changelog-http-cache ferrarimarco/github-changelog-generator --user inviqa --project harness-base-php --exclude-labels "duplicate,question,invalid,wontfix,skip-changelog" --release-branch 0.10.x --future-release 0.10.2
  ```
...
7. Adjust the version for each framework's README.md: `sed -i'' s/v0\.10\.1/v0.10.2/ src/*/README.md src/*/docs/*.md  src/*/docs/*/*.md`
...
1. Tag the release version with `git tag 0.10.2 -m "v0.10.2"`
2. Push the tag to the repository: `git push origin 0.10.2`
...
6. Create a "Github Release" for this repository and downstream repositories, pasting in the changelog for the release:
   - https://github.com/inviqa/harness-base-php/releases/new?tag=0.10.2&title=0.10.2&target=0.10.x
   - https://github.com/inviqa/harness-php/releases/new?tag=0.10.2&title=0.10.2&target=0.10.x
   - https://github.com/inviqa/harness-akeneo/releases/new?tag=0.10.2&title=0.10.2&target=0.10.x
   - https://github.com/inviqa/harness-drupal8/releases/new?tag=0.10.2&title=0.10.2&target=0.10.x
   - https://github.com/inviqa/harness-magento1/releases/new?tag=0.10.2&title=0.10.2&target=0.10.x
   - https://github.com/inviqa/harness-magento2/releases/new?tag=0.10.2&title=0.10.2&target=0.10.x
   - https://github.com/inviqa/harness-spryker/releases/new?tag=0.10.2&title=0.10.2&target=0.10.x
   - https://github.com/inviqa/harness-symfony/releases/new?tag=0.10.2&title=0.10.2&target=0.10.x
   - https://github.com/inviqa/harness-wordpress/releases/new?tag=0.10.2&title=0.10.2&target=0.10.x
...
2. Adjust references from 0.10.x to 0.11.x:
  ...
  ```
3. Adjust references in this file from 0.10.2 to 0.11.0:
  ```bash
  grep -FR '0.10.2' README.md
